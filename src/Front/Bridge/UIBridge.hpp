...
public slots:
    ...
    Q_INVOKABLE void changePatternBackEnd(int patternIndex, int gridIndex) const noexcept;

private:
    [[nodiscard]] constexpr std::chrono::microseconds calculateWaitTimeFromSlider() const noexcept
    {
        // Time calculated in seconds
        const auto calculatedWaitTime = MainGridModel::calculateLinearValue(_gameSpeed, WAIT_MAX, WAIT_MIN);
