...
QVariant UIGrid::data(const QModelIndex &index, int role) const
{
    ...
    int vectorIndex = (row - _gridFirstRow) * _columnCount + (column - _gridFirstColumn);


    if (row >= _gridFirstRow && row < _gridFirstRow + _lineCount &&
        column >= _gridFirstColumn && column < _gridFirstColumn + _columnCount &&
        vectorIndex >= 0 && vectorIndex < _gridUI.size())
    {
        return _gridUI[vectorIndex];
    }*/
    if(const auto vecId{calculateIndexFromUIRow(index)};vecId.has_value()){
        return _gridUI[vecId.value()];
    }
